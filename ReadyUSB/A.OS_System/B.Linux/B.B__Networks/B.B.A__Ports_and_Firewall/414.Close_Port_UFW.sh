...
# Script: 414.Close_Port_UFW.sh
# ScriptID: ST-LIN-0414
# Version: 1.0
# Date: 2026-07-01
# Category: Linux > Networks > Ports And Firewall
# Description: Closes (denies) a port in the UFW firewall.
...
echo -e '\033[0;36m  Script: 414.Close_Port_UFW.sh\033[0m'
...
echo -e '\033[0;36m  Description: Closes (denies) a port in the UFW firewall\033[0m'
...
echo ""
echo -e "${YELLOW}  Denying port $p/$proto in UFW...${NC}"
ufw deny "$p/$proto"

if [ $? -eq 0 ]; then
    echo -e "${GREEN}  SUCCESS: Port $p/$proto denied in UFW.${NC}"
else
